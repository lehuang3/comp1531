...
describe('v1 routes', () => {
  ...
  test('Invalide quiz ID', () => {
    ...
    const response = requestAdminQuizQuestionDuplicateV1(token1, quiz2.quizId, questionId);
    expect(response.body).toStrictEqual({ error: expect.any(String) });
    expect(response.status).toStrictEqual(400);
  });
  ...
  test('Invalide question ID', () => {
    ...
    const response = requestAdminQuizQuestionDuplicateV1(token1, quiz, questionId4.questionId);
    expect(response.body).toStrictEqual({ error: expect.any(String) });
    expect(response.status).toStrictEqual(400);
  });
