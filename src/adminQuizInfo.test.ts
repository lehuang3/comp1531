...
describe('Valid inputs, test pass', () => {
  test('Test 1 correct inputs', () => {
    expect(requestAdminQuizInfo(token1.body, quiz1.body.quizId).body).toMatchObject({
    ...
    expect(requestAdminQuizInfo(token1.body, quiz2.body.quizId).body).toMatchObject({
      quizId: expect.any(Number),
      name: 'quiz123',
      timeCreated: expect.any(Number),
      timeLastEdited: expect.any(Number),
  ...
  test('Test 2 correct inputs', () => {
    expect(requestAdminQuizInfo(token2.body, quiz3.body.quizId).body).toMatchObject({
      quizId: expect.any(Number),
      name: 'quizname1',
      timeCreated: expect.any(Number),
      timeLastEdited: expect.any(Number),
    ...
    expect(requestAdminQuizInfo(token2.body, quiz4.body.quizId).body).toMatchObject({
      quizId: expect.any(Number),
      name: 'quizname',
      timeCreated: expect.any(Number),
      timeLastEdited: expect.any(Number),
...
describe('Invalid quizId', () => {
  test('Test 1 invalid quizId', () => {
    expect(requestAdminQuizInfo(token1.body, -1).body).toStrictEqual({ error: 'Quiz does not exist.' })
  })
  test('Test 2 invalid quizId', () => {
    expect(requestAdminQuizInfo(token2.body, -2).body).toStrictEqual({ error: 'Quiz does not exist.' })
  })
})

describe('No permission to view quiz', () => {
  test('Test 1, two users trying to access quizzes they do not own', () => {
    expect(requestAdminQuizInfo(token1.body, quiz3.body.quizId).body).toStrictEqual({ error: 'You do not have access to this quiz.' })
  })
  test('Test 2, two users trying to access quizzes they do not own', () => {
    expect(requestAdminQuizInfo(token1.body, quiz4.body.quizId).body).toStrictEqual({ error: 'You do not have access to this quiz.' })
  })
  test('Test 3, two users trying to access quizzes they do not own', () => {
    expect(requestAdminQuizInfo(token2.body, quiz1.body.quizId).body).toStrictEqual({ error: 'You do not have access to this quiz.' })
  })
  test('Test 4, two users trying to access quizzes they do not own', () => {
    expect(requestAdminQuizInfo(token2.body, quiz2.body.quizId).body).toStrictEqual({ error: 'You do not have access to this quiz.' })
  })
})
...
describe('Invalid token', () => {
  test('Invalid token created from invalid email', () => {
    ...
    expect(requestAdminQuizInfo(invalidToken1.body, quiz1.body.quizId).body).toStrictEqual({ error: 'Invalid token structure' })
  })
  test('Invalid token created from invalid password', () => {
    ...
    expect(requestAdminQuizInfo(invalidToken2.body, quiz1.body.quizId).body).toStrictEqual({ error: 'Invalid token structure' })
  })
})