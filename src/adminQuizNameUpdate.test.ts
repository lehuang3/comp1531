...
describe('v1 routes', () => {
  describe('Passing cases', () => {
    ...
    test('User 2 changes quiz name to valid quiz name', () => {
      expect(v1requestAdminQuizNameUpdate(token2, quiz2, 'abc').body).toStrictEqual({ });
    });
  });
  ...
  describe('authUserId is not valid', () => {
    ...
    test('User 2 tries to change user 1 quiz name', () => {
      expect(v1requestAdminQuizNameUpdate(token2, quiz1, 'quiz2').body).toStrictEqual({ error: 'You do not have access to this quiz.' });
    });
  });

  describe('quizId is not valid', () => {
    test('User 1 negative quizId not valid', () => {
      expect(v1requestAdminQuizNameUpdate(token1, -1, 'quiz2').body).toStrictEqual({ error: 'Quiz does not exist.' });
    });
    test('User 2 negative quizId not valid', () => {
      expect(v1requestAdminQuizNameUpdate(token2, -2, 'quiz2').body).toStrictEqual({ error: 'Quiz does not exist.' });
    });
  });

  describe('Quiz name is not valid', () => {
    test('User 1 quiz name not valid', () => {
      expect(v1requestAdminQuizNameUpdate(token1, quiz1, 'quiz#').body).toStrictEqual({ error: 'Quiz name cannot have special characters.' });
    });
    test('User 2 quiz name not valid', () => {
      expect(v1requestAdminQuizNameUpdate(token2, quiz2, 'ad12_131').body).toStrictEqual({ error: 'Quiz name cannot have special characters.' });
    });
  });

  describe('Quiz name too long or short', () => {
    test('User 1 quiz too short', () => {
      expect(v1requestAdminQuizNameUpdate(token1, quiz1, 'q1').body).toStrictEqual({ error: 'Quiz name must be greater or equal to 3 chartacters and less than or equal to 30.' });
    });
    test('User 2 quiz too long', () => {
      expect(v1requestAdminQuizNameUpdate(token2, quiz2, '1231245523414535234115234541352562134265afasf').body).toStrictEqual({ error: 'Quiz name must be greater or equal to 3 chartacters and less than or equal to 30.' });
    });
  });
  ...
  describe('Quiz name already used', () => {
    test('User 1 quiz name already used', () => {
      ...
      expect(v1requestAdminQuizNameUpdate(token1, quiz1, 'newquiz').body).toStrictEqual({ error: 'Quiz name already exists.' });
    });
    test('User 2 quiz name already used', () => {
      requestAdminQuizCreate(token2, 'newquiz1', 'quiz1');
      expect(v1requestAdminQuizNameUpdate(token2, quiz2, 'newquiz1').body).toStrictEqual({ error: 'Quiz name already exists.' });
    });
  });
  ...
  describe('Quiz in trash', () => {
    test('User 1 changes quiz name to valid quiz name', () => {
      ...
      expect(v1requestAdminQuizNameUpdate(token1, quiz1, 'quiz2').body).toStrictEqual({ error: 'Quiz is in trash.' });
    });
  });
});
