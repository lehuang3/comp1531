...
describe('v1 routes', () => {
  ...
  describe('No ownership of quiz', () => {
    test('token1 attempts to restore quiz owned by 2', () => {
      ...
      expect(v1requestAdminQuizRestore(token1, quiz2).body).toStrictEqual({ error: 'You do not have access to this quiz.' });
    });
  });
  ...
  describe('Quiz not in trash', () => {
    test('quiz 2 not in trash', () => {
      ...
      expect(v1requestAdminQuizRestore(token1, quiz2).body).toStrictEqual({ error: 'Quiz not in trash.' });
    });
  });
